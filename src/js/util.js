...
export function reversed(arr) {
  let res = [];
  for (let j = arr.length - 1; j > -1; j--) res.push(arr[j]);
  return res;
}
...
export function bilinear(x, y, grid, default_value) {
  ...
  let j = Math.floor(y);
  let get_grid = (j, i) =>
    i >= 0 && i < n_cols && j >= 0 && j < n_rows && isFinite(grid[j][i])
      ? grid[j][i]
      : undefined;
  ...
  y -= j;
  return (
    f00 + (f10 - f00) * x + (f01 - f00) * y + (f11 - f10 - f01 + f00) * x * y
  );
}
...
export function point_in_lake_tahoe(point) {
  ...
  if (p_string in point_lake_cache) return point_lake_cache[p_string];
...
export function points_in_lake_tahoe(width, height) {
  ...
  if (key in lake_points_cache) return lake_points_cache[key];
  const res = [];
  for (let j = 0; j < height; j++)
    for (let i = 0; i < width; i++)
      if (point_in_lake_tahoe([i / width, 1 - j / height])) res.push([i, j]);
...
export function draw_lake_heatmap(canvas, heatmap_data, color_palette, key) {
  ...
  cx.putImageData(image_data, offsetX, offsetY);
  if (key !== undefined) heatmap_cache[key] = image_data;
}
...
export function apply(array, callback_fn) {
  ...
  for (let [idx, value] of array.entries()) {
    if (Array.isArray(value)) apply(value, callback_fn);
    else array[idx] = callback_fn(value);
  }
  return array;
}
...
export function zip(...arrays) {
  ...
  for (let i = 0; i < min_length; i++) {
    let tuple = [];
    for (let j = 0; j < arrays.length; j++) tuple.push(arrays[j][i]);
    zipped.push(tuple);
  }
  return zipped;
}
...
export function unzip(arr) {
  ...
  let res = [];
  for (let i = 0; i < arr[0].length; i++) res.push([]);
  for (let tuple of arr) {
    for (let [idx, value] of tuple.entries()) res[idx].push(value);
  }
  return res;
}
...
export function mean(arr, k) {
  ...
  if (k < 1 || k > arr.length)
    throw new Error(
      `mean(): Expected moving window size k to be 1 <= k <= arr.length, got k=${k}`
    );

  let res = [0];
  for (let i = 0; i < k; i++) res[0] += arr[i];
  res[0] /= k;
...
export function wind_direction_mean(wd_vector, k, units, nan_var) {
  ...
  if (nan_var === "nanmean" && wd_vector.some(isNaN)) return Number.NaN;
  else wd_vector = wd_vector.filter(isFinite);
  ...
  if (k === wd_vector.length) return mean_wd[0];
  return mean_wd;
}
